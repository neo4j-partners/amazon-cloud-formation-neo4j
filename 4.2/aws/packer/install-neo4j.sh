...
echo 'deb http://debian.neo4j.com stable 4.2' | sudo tee -a /etc/apt/sources.list.d/neo4j.list
...
if [ $neo4j_edition = "community" ]; then
    ...
    sudo apt-get --yes install neo4j=$neo4j_version cypher-shell=4.2.1
else
    echo "neo4j-enterprise=$neo4j_version"
    sudo apt-get --yes install neo4j-enterprise=$neo4j_version cypher-shell=4.2.1
fi
