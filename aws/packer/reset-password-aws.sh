...
# DEPRECATED RESET PASSWORD METHOD
...
#      break
#  fi
sudo /usr/bin/neo4j-admin set-initial-password $AWSINSTANCEID

if [ $? -eq 0 ] ; then
  echo `date` "Default password reset to AWS instance ID $AWSINSTANCEID - a graph user is you!" | sudo tee $LOGFILE
else
  echo "Password has already been reset" | sudo  tee $LOGFILE
fi

echo `date` 'Waiting for neo4j to come up' | sudo tee $LOGFILE
sleep 1