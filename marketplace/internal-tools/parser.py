...
parser.add_argument('--ami-owners', default='137112412989', help='A comma separate list of account ids.')
...
cmd = subprocess.run(
  ["packer", "build",
   ...
   "-var", f"ami_owners{args.ami_owners}",
...
with open("../neo4j.template_copy.yaml", "r") as file:
  template = file.read()
  ami_index = template.find("Mappings:\n  Neo4j:\n")
  updated_template = f'{template[:ami_index]}\n{output}'

with open("../neo4j.template_copy.yaml", "w") as file:
