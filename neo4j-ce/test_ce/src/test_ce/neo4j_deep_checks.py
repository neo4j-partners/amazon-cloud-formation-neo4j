...
def check_server_status(config: StackConfig, reporter: TestReporter) -> None:
    """Verify Neo4j reports itself as Community Edition via dbms.components()."""
        ...
        try:
            with config.driver() as driver:
                ...
                if "community" in edition.lower():
                    ...
                    ctx.fail(f"Expected community edition, got: {edition}")
...
def check_advertised_address(config: StackConfig, reporter: TestReporter) -> None:
    """Verify the advertised address matches the Elastic IP."""
