...
For a production-grade certificate, see the [Bolt TLS section below](#bolt-tls) for the customer-owned DNS pattern and cert rotation approach.

### Access via Bastion

Instances have no public IP. Operator access goes through the `t4g.nano` bastion via SSM Session Manager port-forwarding.

The stack outputs include ready-to-run commands with the bastion instance ID and NLB DNS already substituted:
...
Run the two tunnel commands in separate terminals (or backgrounded with `&`). Then:
...
> **Note:** Writes through Neo4j Browser go to whichever node the NLB selects. That node may not be the leader, which produces a `NotALeader` error. Use the admin shell for writes.
...
Opens `http://localhost:7474` in a browser. The tunnel connects to the NLB on port 7474; the NLB routes each new TCP connection to a cluster node.
...
Deletes the stack, force-deletes the Neo4j password secret and Bolt TLS cert secret (to unblock same-name redeployment), removes the staged `lambda/neo4j-ca.crt`, and removes `.deploy/<stack-name>.txt`. EBS data volumes survive by design.

```bash
./teardown.sh --delete-volumes     # also permanently deletes EBS volumes
```

If the sample private app is deployed, tear it down first. It owns security group ingress rules on the EE stack and will stall stack deletion if left in place:
...
| Step | What it checks |
|---|---|
| `cloudwatch` | CloudWatch agent active on all nodes |
| `logs` | Application log group exists with expected stream count |
...
See [APP_GUIDE.md](../APP_GUIDE.md) for the full application connection pattern.
