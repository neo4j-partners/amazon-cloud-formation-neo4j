...
| Bolt TLS | Optional | Set `BoltCertificateSecretArn` to a Secrets Manager secret containing `certificate` and `private_key` PEM fields. When set, Neo4j enables `server.bolt.tls_level=REQUIRED`. Leave blank for plaintext Bolt. |
