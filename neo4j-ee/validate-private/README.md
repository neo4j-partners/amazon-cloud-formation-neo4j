...
| `scripts/browser-tunnel.sh [stack]` | Open a port-forward tunnel to the NLB on port 7473 (HTTPS). After mapping AdvertisedDNS to 127.0.0.1 in /etc/hosts, go to `https://<AdvertisedDNS>:7473`. | Interactive |
| `scripts/bolt-tunnel.sh [stack]` | Open a port-forward tunnel to the NLB on port 7687 (Bolt). Connect a local driver or client to the URI printed by the script: `neo4j+s://<AdvertisedDNS>:7687` for trusted certs, or `neo4j+ssc://<AdvertisedDNS>:7687` for self-signed test certs. Requires `127.0.0.1 <AdvertisedDNS>` in `/etc/hosts`. | Interactive |
| `scripts/ssm_check_sessions.sh [stack]` | List active SSM sessions for the stack's Neo4j instance(s) and operator bastion. | <5s |
